...
- Сервер больше не завершается при отсутствии `WORDSTAT_API_KEY` / `WORDSTAT_FOLDER_ID`:
  он стартует, отвечает на `initialize` и `tools/list`, а в `instructions` предупреждает
  модель, что оператор должен задать переменные окружения и перезапустить сервер.
  Прежние тексты стартовых ошибок («Требуется WORDSTAT_API_KEY…», «Требуется
  WORDSTAT_FOLDER_ID…») сохранены дословно и возвращаются теперь при вызове инструмента
  (`CredentialsError`, ответ с `isError`) — без ретраев и без обращения к сети. Раньше
  процесс умирал до MCP-хендшейка, и пользователь видел только «красный крест» без причины.
- Телеметрия: у выжившего неконфигурированного старта своё событие `unconfigured_start`
  с прежними кодами причин (`missing_token` / `missing_folder_id`); `server_start` теперь
  означает старт рабочей установки. `startup_failed` остаётся для невалидных значений
  конфигурации и отправляется fire-and-forget — процесс больше не ждёт пинг перед выходом.
